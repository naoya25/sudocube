...
クリア時のスコアは 0〜100 の実数（小数第3位まで表示）で、次の式で計算する:

```
score = max(0.001, 100 × 0.97^min(ミス回数, 3) × 0.90^max(0, ミス回数 − 3) × (1800 / (1800 + 経過秒))^0.5)
```

- **ミス**: 2段階ペナルティ。3回までは 1 回ごとに残りスコアの 3% 減、4回目からは 10% 減（いずれも乗算なのでどれだけミスしても 0 点にはならない）
- **タイム**: 経過 0 秒から効く凸カーブで、速いほど 1 秒の価値が高い（序盤は 1 秒 ≈ 0.026 点、2 時間地点では ≈ 0.003 点）
...
定数は [src/core/session.ts](src/core/session.ts) の `MISTAKE_RETENTION_SOFT` / `MISTAKE_RETENTION_HARD` / `MISTAKE_SOFT_LIMIT` / `TIME_SCALE_SECONDS` / `TIME_DECAY_EXPONENT` で調整できる。
